...
import { parse, CircuitDrawing, Circuit } from "../../../crates/js/pkg/quilla_js.js"
...
function drawCircuit(parsed: ParseItem[], qbits: number, cbits: number): string {
  if (qbits === 0) return ""
...
function sampleCircuit(parsed: ParseItem[], qbits: number, cbits: number): Map<string, number> | null {
  if (qbits === 0 || cbits === 0) return null
  const circuit = new Circuit(qbits, cbits)
  for (const item of parsed) {
    if ("operation" in item) {
